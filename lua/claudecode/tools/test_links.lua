...
local function handler(params)
  ...
  if include_diagnostics then
    ...
    table.insert(test_output, "🔤 SYMBOL/VARIABLE REFERENCES (WITH FULL PATHS - CLICKABLE!):")
    table.insert(test_output, "• The function `handle_link_click` is defined in `lua/claudecode/links.lua:357`")
    table.insert(test_output, "• Variable `config` is initialized in `lua/claudecode/config.lua:10`")
    table.insert(test_output, "• Check the `parse_file_reference` function in `lua/claudecode/links.lua:90`")
    table.insert(test_output, "• The `M.setup` method is in `lua/claudecode/init.lua:322`")
    table.insert(test_output, "• Method `vim.api.nvim_buf_get_lines` - see Neovim API docs")
    table.insert(test_output, "• State variable `setup_buffers` in `lua/claudecode/terminal_links.lua:15`")
    table.insert(test_output, "• Logger function `logger.debug` used in `lua/claudecode/logger.lua:45`")
    table.insert(test_output, "")
  end
  
  table.insert(test_output, "⚡ CLICKABILITY DEMONSTRATION:")
  table.insert(test_output, "Notice how variables now include FULL FILE PATHS!")
  table.insert(test_output, "This makes them directly clickable to jump to definitions.")
  table.insert(test_output, "")
  ...
  table.insert(test_output, "• File references (like `file.py:42`) are directly clickable")
  table.insert(test_output, "• URLs open in your default browser")
  table.insert(test_output, "• Everything with file paths becomes navigable!")
  table.insert(test_output, "")
